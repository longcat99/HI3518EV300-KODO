...
static struct hi3518ev300_spi hisilicon_spi;
...
static int hi3518ev300_spi_setup(struct spi_device *spi)
{
	hisilicon_debug("%s\n",__func__);
	ssp_set();
	return 0;
}
static void hi3518ev300_spi_set_cs(struct spi_device *spi, bool gpio_level)
{
	hisilicon_debug("%s\n",__func__);

}
static int hi3518ev300_spi_transfer_one(struct spi_master *master,
				    struct spi_device *spi,
				    struct spi_transfer *tfr)
{
	struct hi3518ev300_spi *hs = spi_master_get_devdata(master);
	int i;
...
static void hi3518ev300_spi_handle_err(struct spi_master *master,
				   struct spi_message *msg)
{
	hisilicon_debug("%s\n",__func__);
}
static int hi3518ev300_spi_prepare_message(struct spi_master *master,
				       struct spi_message *msg)
{
	hisilicon_debug("%s\n",__func__);
	return 0;
}
static irqreturn_t hi3518ev300_spi_interrupt(int irq, void *dev_id)
{
	hisilicon_debug("%s\n",__func__);
	return IRQ_HANDLED;
}

static int hi3518ev300_spi_probe(struct platform_device *pdev)
{
	
	struct spi_master *master;
	struct hi3518ev300_spi *hs;
	struct resource *res;
	int err;
	hisilicon_debug("%s\n",__func__);
	...
	master->setup = hi3518ev300_spi_setup;
	//master->set_cs = hi3518ev300_spi_set_cs;
	master->transfer_one = hi3518ev300_spi_transfer_one;
	master->handle_err = hi3518ev300_spi_handle_err;
	//master->prepare_message = hi3518ev300_spi_prepare_message;
	...
	err = devm_request_irq(&pdev->dev, hs->irq, hi3518ev300_spi_interrupt, 0,
...
static int hi3518ev300_spi_remove(struct platform_device *pdev)
{
...
static const struct of_device_id hi3518ev300_spi_match[] = {
	{ .compatible = "hi3518ev300-spi", },
	{}
};

MODULE_DEVICE_TABLE(of, hi3518ev300_spi_match);

static struct platform_driver  hi3518ev300_spi_driver = {
	.probe		= hi3518ev300_spi_probe,
	.remove		= hi3518ev300_spi_remove,
	.driver		= {
		.name	= "hi3518ev300-spi",
		.of_match_table	= of_match_ptr(hi3518ev300_spi_match),
		//.owner  = THIS_MODULE,
	},
};

#if 0
static int __init hi3518ev300_spi_init(void)
{
	hisilicon_debug("%s\n",__func__);
	return spi_register_driver(&hi3518ev300_spi_driver);
}
static void __exit hi3518ev300_spi_exit(void)
{
	hisilicon_debug("%s\n",__func__);
	spi_unregister_driver(&hi3518ev300_spi_driver);
}
module_init(hi3518ev300_spi_init);
module_exit(hi3518ev300_spi_exit);
#endif

module_platform_driver(hi3518ev300_spi_driver);
//module_spi_driver(hi3518ev300_spi_driver);

MODULE_DESCRIPTION("hi3518ev300 SPI Driver");
